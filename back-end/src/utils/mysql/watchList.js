...
const { WATCH_LIST_TABLE, TITLES_TABLE, WATCH_HISTORY_TABLE } = process.env;

exports.getWatchList = async (user_id) => {
  try {
    return await query(`
      SELECT ${TITLES_TABLE}.*, ${WATCH_HISTORY_TABLE}.watched_percentage FROM ${WATCH_LIST_TABLE} 
      LEFT JOIN ${TITLES_TABLE} ON  ${WATCH_LIST_TABLE}.title_id = ${TITLES_TABLE}.id
      LEFT JOIN ${WATCH_HISTORY_TABLE}
      ON ${TITLES_TABLE}.id = ${WATCH_HISTORY_TABLE}.title_id AND "${user_id}" = ${WATCH_HISTORY_TABLE}.user_id
      WHERE ${WATCH_LIST_TABLE}.user_id = '${user_id}'
